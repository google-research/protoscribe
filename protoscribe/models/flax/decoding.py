...
class DecodingState:
  ...
  cur_index: chex.Array
  sequences: chex.Array
  cur_token: chex.Array
  cache: dict[str, chex.Array]
...
LogitCallbackFn = Callable[[chex.Array, SamplingLoopState], chex.Array]
...
def temperature_sample(
    ...
    tokens_to_logits: Callable[
        [DecodingState], tuple[chex.Array, dict[str, chex.Array]]
    ],
    eos_id: int,
    decode_rng: Optional[chex.Array] = None,
    num_decodes: int = 1,
    temperature: Union[float, chex.Array] = 1.0,
    topk: int = 1,
    topp: float = 0.0,
    cache_offset: int = 0,
    initial_index: Optional[chex.Array] = None,
    max_decode_steps: Optional[Union[int, chex.Array]] = None,
...
def _temperature_sample_single_trial(
    ...
    temperature: Union[float, chex.Array] = 1.0,
    topk: int = 20,
    topp: Union[float, chex.Array] = 0.0,
    initial_index: Optional[chex.Array] = None,
    max_decode_steps: Optional[Union[int, chex.Array]] = None,
    rescale_log_probs: bool = True,
    state_callback_fn: Optional[StateCallbackFn] = None,
  ...
  def sampling_loop_cond_fn(state: SamplingLoopState) -> chex.Array:
...
def brevity_penalty(alpha: float, length: int) -> chex.Array:
...
def add_beam_dim(
    x: chex.Array, beam_size: int, offset: int = 0
) -> chex.Array:
...
def flatten_beam_dim(x: chex.Array, offset: int = 0) -> chex.Array:
...
def unflatten_beam_dim(
    x: chex.Array, batch_size: int, beam_size: int, offset: int = 0
) -> chex.Array:
...
class BeamState:
  ...
  cur_index: chex.Array  # scalar int32: current decoded length index
  # The active sequence log probabilities and finished sequence scores.
  live_logprobs: chex.Array  # float32: [batch_size, beam_size]
  finished_scores: chex.Array  # float32: [batch_size, beam_size]
  # The current active-beam-searching and finished sequences.
  live_seqs: chex.Array  # int32: [batch_size, beam_size, max_decode_len]
  finished_seqs: chex.Array  # int32: [batch_size, beam_size,
  #                                         max_decode_len]
  # Records which of the 'finished_seqs' is occupied and not a filler slot.
  finished_flags: chex.Array  # bool: [batch_size, beam_size]
...
def beam_init(
    ...
    cache: dict[str, chex.Array],
    offset: int = 0,
    live_seqs: Optional[chex.Array] = None,
    initial_index: Optional[chex.Array] = None,
) -> BeamState:
...
def beam_search(
  ...
  TODO: Solve this, if needed, by having a third class of sequences
  ...
  Args:
    ...
    cache_offset: axis offset for cache, arising from scanned layers.
    initial_index: Optional[chex.Array], the index from which to start decoding
  ...
  def beam_search_loop_cond_fn(state: BeamState) -> chex.Array:
