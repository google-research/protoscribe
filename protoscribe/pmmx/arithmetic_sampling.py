...
Sumit Sanghai: "Arithmetic Sampling: Parallel Diverse Decoding for Large
Language Models", https://arxiv.org/abs/2210.15458.

Cloned from https://github.com/google-research/google-research/tree/master/arithmetic_sampling. # pylint: disable=line-too-long
"""
...
def _sequential_cumsum(arr: jnp.ndarray, axis: int) -> jnp.ndarray:
...
def _arithmetic_categorical(
    rng: jnp.ndarray, logits: jnp.ndarray,
    codes: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
...
def arithmetic_sample(
    inputs: jnp.ndarray,
    cache: Mapping[str, jnp.ndarray],
    tokens_to_logits: Callable[[decoding.DecodingState],
    ...
    state_callback_fn: Optional[Callable[[ArithmeticSamplingLoopState],
                                         ArithmeticSamplingLoopState]] = None,
    logit_callback_fn: Optional[Callable[
        [jnp.ndarray, ArithmeticSamplingLoopState], jnp.ndarray]] = None
...
def _make_default_codes(batch_size: int, num_decodes: int,
...
def _arithmetic_sample_single_trial(
  ...
  def sampling_loop_cond_fn(state: ArithmeticSamplingLoopState) -> bool:
  ...
  def sampling_loop_body_fn(
      state: ArithmeticSamplingLoopState) -> ArithmeticSamplingLoopState:
